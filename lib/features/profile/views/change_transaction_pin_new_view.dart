...
class ChangeTransactionPinNewView extends ConsumerStatefulWidget {
  const ChangeTransactionPinNewView({super.key});

  @override
  ConsumerState<ChangeTransactionPinNewView> createState() =>
      _ChangeTransactionPinNewViewState();
}

class _ChangeTransactionPinNewViewState
    extends ConsumerState<ChangeTransactionPinNewView> {
  ...
  Future<void> _handleContinue() async {
    ...
    await _secureStorage.write('temp_new_transaction_pin', _localPin);

    // Navigate to confirm PIN view
    appRouter.pushNamed(AppRoute.changeTransactionPinConfirmView);
  }
                  ...
                  child: PrimaryButton(
                    ...
                    onPressed:
                        _localPin.length == 4 && _errorMessage.isEmpty
                            ? _handleContinue
                            : null,
                    enabled: _localPin.length == 4 && _errorMessage.isEmpty,
                    backgroundColor:
                        _localPin.length == 4 && _errorMessage.isEmpty
