...
import 'package:dayfi/core/network/api_error.dart';
import 'package:dayfi/core/network/app_interceptor.dart';
...
class NetworkService {
  ...
  Future<Options> getOption({responseType, bool isJson = true}) async {
    String token = await localCache.getToken();
    return isJson
        ? Options(responseType: responseType, headers: {"Authorization": token.isNotEmpty ? "Bearer $token" : ''})
        : Options(
    ...
    try {
      switch (method) {
        ...
        case RequestMethod.get:
          response = await dio!.get(path, queryParameters: params, options: await getOption());
          break;
        case RequestMethod.patch:
          response = await dio!.patch(path, queryParameters: params, data: data, options: await getOption());
          break;
        case RequestMethod.put:
          response = await dio!.put(path, queryParameters: params, data: data, options: await getOption());
          break;
        case RequestMethod.delete:
          response = await dio!.delete(path, queryParameters: params, data: data, options: await getOption());
          break;
